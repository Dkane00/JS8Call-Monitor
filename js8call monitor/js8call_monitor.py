...
class tcp_client:
    """ Class TCP Client to connect to JS8Call TCP API and push JSON messages to msgQueue """
    config = None
    js8host = ""
    js8port = 0
    poll_interval = 5
    ...
    def __init__(self):
        ...
        # read settings from [TCP] section (add these keys to your config)
        # enabled = true/false
        # host = 127.0.0.1
        # port = 2171
        # poll_status_interval = 5
        try:
            self.enabled = bool(strtobool(self.config['TCP']['enabled']))
        except Exception:
            self.enabled = False
        try:
            self.js8host = self.config['TCP']['host']
            self.js8port = int(self.config['TCP']['port'])
        except Exception:
            self.js8host = '127.0.0.1'
            self.js8port = 2171
        try:
            self.poll_interval = int(self.config['TCP'].get('poll_status_interval', 5))
        except Exception:
            self.poll_interval = 5

        print("tcp_client: Host: %s Port: %s Enabled: %s" % (self.js8host, self.js8port, self.enabled))
        if self.enabled:
            t = threading.Thread(name='tcp_client', target=self.run)
            t.daemon = True
            t.start()
            print("tcp_client: Started > %s" % t.is_alive())

    def run(self):
        """ Connect to JS8Call TCP API, read lines, and push to msgQueue """
        while True:
            sock = None
            try:
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.settimeout(10)
                sock.connect((self.js8host, int(self.js8port)))
                # wrap socket as file-like to read newline-terminated JSON messages
                if sys.version_info.major < 3:
                    sf = sock.makefile('r')
                else:
                    sf = sock.makefile('r', encoding='utf-8', newline='\n')
                print("tcp_client: Connected to %s:%s" % (self.js8host, self.js8port))
                ...
                while True:
                    ...
                    if not line:
                        ...
                        print("tcp_client: Connection closed by remote")
                        break
                    # strip trailing newline(s)
                    data = line.strip()
                    if data == "":
                        continue
                    # Format message to match UDP server style: "('host', port)|<payload>"
                    addr = (self.js8host, self.js8port)
                    try:
                        msg = str(addr) + "|" + data
                        msgQueue.put(msg)
                    except Exception as e:
                        print("tcp_client: Failed to queue message: %s" % e)
                try:
                    sf.close()
                except Exception:
                    pass
                ...
                print("tcp_client: Socket error: %s" % e)
            finally:
                try:
                    if sock:
                        sock.close()
...
class JS8CallMonitor:
    ...
    def parse_mode(self):
        """ Parse MODE and set icons """
        # pylint: disable=too-many-branches
        # pylint: disable=too-many-statements
        # It's a parser after all
        self.debug_log(self.log_entry,"parse_mode: --Enter--")
        self.debug_log(self.log_info,"parse_mode: Parsing MODE tag from JS8Call...")
        mode = self.adif_MODE
        if not mode:                
            # set NOINFO
            # This indicates the MODE field was blank
            attr = self.mapnoinfo
            tag, value = attr.strip().split(':')
            tag = "adif_" + tag
            setattr(self, tag, value)
            self.adif_QSO_COLOR = str(self.mapnoinfocolor)
            self.adif_QSO_ICON = str(self.mapnoinfoaprs)
            self.debug_log(self.log_info,"parse_mode: No MODE detected")
            self.debug_log(self.log_debug,"parse_mode: NOINFO %s: %s (%s) (%s)" % (tag, value, self.adif_QSO_COLOR, self.adif_QSO_ICON))
            self.debug_log(self.log_entry,"parse_mode: --Exit--")
            return
        self.debug_log(self.log_debug,"parse_mode: MODE detected")
        # Check MODE for NOINFO tag
        attr = self.mapnoinfo
        tag, value = attr.strip().split(':')
        tag = "adif_" + tag
        if mode == value:
            self.adif_QSO_COLOR = str(self.mapnoinfocolor)
            self.adif_QSO_ICON = str(self.mapnoinfoaprs)
            self.debug_log(self.log_info,"parse_mode: Set NOINFO")
            self.debug_log(self.log_debug,"parse_mode: NOINFO %s: %s (%s) (%s)" % (tag, value, self.adif_QSO_COLOR, self.adif_QSO_ICON))
            self.debug_log(self.log_entry,"parse_mode: --Exit--")
            return
        # Check MODE for NOPIR tag
        attr = self.mapnopir
        tag, value = attr.strip().split(':')
        tag = "adif_" + tag
        if mode == value:
            self.adif_QSO_COLOR = str(self.mapnopircolor)
            self.adif_QSO_ICON = str(self.mapnopiraprs)
            self.debug_log(self.log_info,"parse_mode: Set NOPIR")
            self.debug_log(self.log_debug,"parse_mode: NOPIR %s: %s (%s) (%s)" % (tag, value, self.adif_QSO_COLOR, self.adif_QSO_ICON))
            self.debug_log(self.log_entry,"parse_mode: --Exit--")
            return
        # check MODE for UNKNOWN tag
        attr = str(self.config['UNKNOWN']['map'])
        tag, value = attr.strip().split(':')
        tag = "adif_" + tag
        if mode == value:
            self.adif_QSO_COLOR = str(self.config['UNKNOWN']['color'])
            self.adif_QSO_ICON = str(self.config['UNKNOWN']['aprs'])
            self.debug_log(self.log_info,"parse_mode: Set UNKNOWN")
            self.debug_log(self.log_debug,"parse_mode: UNKNOWN %s: %s (%s) (%s)" % (tag, value, self.adif_QSO_COLOR, self.adif_QSO_ICON))
            self.debug_log(self.log_entry,"parse_mode: --Exit--")
            return
        # check MODE for GREEN tag
        attr = str(self.config['GREEN']['map'])
        tag, value = attr.strip().split(':')
        tag = "adif_" + tag
        if mode == value:
            self.adif_QSO_COLOR = str(self.config['GREEN']['color'])
            self.adif_QSO_ICON = str(self.config['GREEN']['aprs'])            
            self.debug_log(self.log_info,"parse_mode: Set GREEN")
            self.debug_log(self.log_debug,"parse_mode: GREEN %s: %s (%s) (%s)" % (tag, value, self.adif_QSO_COLOR, self.adif_QSO_ICON))
            self.debug_log(self.log_entry,"parse_mode: --Exit--")
            return
        # check MODE for YELLOW tag
        attr = str(self.config['YELLOW']['map'])
        tag, value = attr.strip().split(':')
        tag = "adif_" + tag
        if mode == value:
            self.adif_QSO_COLOR = str(self.config['YELLOW']['color'])
            self.adif_QSO_ICON = str(self.config['YELLOW']['aprs'])
            self.debug_log(self.log_info,"parse_mode: Set YELLOW")
            self.debug_log(self.log_debug,"parse_mode: YELLOW %s: %s (%s) (%s)" % (tag, value, self.adif_QSO_COLOR, self.adif_QSO_ICON))
            self.debug_log(self.log_entry,"parse_mode: --Exit--")
            return
        # check MODE for RED tag
        attr = str(self.config['RED']['map'])
        tag, value = attr.strip().split(':')
        tag = "adif_" + tag
        if mode == value:
            self.adif_QSO_COLOR = str(self.config['RED']['color'])
            self.adif_QSO_ICON = str(self.config['RED']['aprs'])
            self.debug_log(self.log_info,"parse_mode: Set RED")
            self.debug_log(self.log_debug,"parse_mode: RED %s: %s (%s) (%s)" % (tag, value, self.adif_QSO_COLOR, self.adif_QSO_ICON))
            self.debug_log(self.log_entry,"parse_mode: --Exit--")
            return
        # check MODE for STATUS tag
        attr = self.mapstatus
        tag, value = attr.strip().split(':')
        tag = "adif_" + tag
        if mode == value:
            self.adif_QSO_COLOR = str(self.mapstatuscolor)
            self.adif_QSO_ICON = str(self.mapstatusaprs)
            self.debug_log(self.log_info,"parse_mode: Set STATUS")
            self.debug_log(self.log_debug,"parse_mode: STATUS %s: %s (%s) (%s)" % (tag, value, self.adif_QSO_COLOR, self.adif_QSO_ICON))
            self.debug_log(self.log_entry,"parse_mode: --Exit--")
            return
        # MODE did not match anything defined in the config, so set defaults.
        self.adif_QSO_COLOR = str(self.mapnoinfocolor)
        self.adif_QSO_ICON = str(self.mapnoinfoaprs)
        self.debug_log(self.log_info,"parse_mode: No match of MODE to config.")
        self.debug_log(self.log_info,"parse_mode: Set NOINFO")
        self.debug_log(self.log_debug,"parse_mode: NOINFO %s: %s (%s) (%s)" % (tag, value, self.adif_QSO_COLOR, self.adif_QSO_ICON))
        self.debug_log(self.log_entry,"parse_mode: --Exit--")
...
if __name__ == "__main__":
    ...
    print("JS8Call Monitor v0.21 written by KK7JND")

    # Instantiate monitor (loads the config)
    J = JS8CallMonitor()

    # Decide whether to use TCP or UDP based on config:
    tcp_enabled = False
    try:
        if 'TCP' in J.config:
            tcp_enabled = bool(strtobool(J.config['TCP'].get('enabled', 'false')))
    except Exception:
        tcp_enabled = False

    if tcp_enabled:
        print("Starting TCP client...")
        # Start TCP client (it will push messages to msgQueue)
        T = tcp_client()
    else:
